...
const solutionsDirPath = path.resolve(process.cwd(), config.outputDir)
const resultJsonPath = path.resolve(solutionsDirPath, 'result.json')
...
exports.fetch = co.wrap(function * (conf, numObj) {
     ...
     * generate READE.md
     * */
    yield generateMDUtils.generateMarkdown(resultObj, leetcodeNumObj, config.outputDir)
})
